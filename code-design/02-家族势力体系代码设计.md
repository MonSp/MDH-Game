...
### 已实现
- `FamilyService` 单例，家族创建层级(1皇族+3一级+5二级+7三级=16/国)
- `FavorabilitySystem` 单例 (好感度初始化、修改、hostile事件)
- `assignPlayerToFamily`, `getFamiliesByCountry`, `getFamilyConfig`

### 未实现 / Bug
- **`getFamilyFavorability` 始终返回50(初始值)**: 未从 `FavorabilitySystem` 读取实际值
- **`modifyFavorability` 每次从50重新计算**: 累积好感度被丢弃
- **`triggerElderHunt` 仅 console.log**: 未生成执法长老NPC
- **`FamilyStrategyLLM`**: 未实现
- 家族间外交/联盟未实现

### 与 docs/ 设计差距
- docs中的7种门派类型(剑/法/体/丹/阵/器/符)未在code-design和代码中体现
- docs中的军团层级(军团→营)未实现
- docs中的AI指挥官人格系统未实现
